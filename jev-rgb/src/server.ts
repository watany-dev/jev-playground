...
import { MAX_TEXT_LENGTH, createReader, errorCategory, readApiKey, type Reader } from './evaluate';
import { page } from './ui';
...
export function createHandler(read: Reader): (request: Request) => Promise<Response> {
  ...
  return async request => {
    ...
    try {
      ...
      return Response.json(await read(text));
    } catch (error) {
...
if (import.meta.main) {
  ...
  const server = Bun.serve({
    hostname: process.env.JEV_RGB_HOST ?? '127.0.0.1',
    port: Number(process.env.JEV_RGB_PORT ?? 8787),
    fetch: handler,
  });
  console.log(`jev-rgb: http://${server.hostname}:${server.port}`);
}
